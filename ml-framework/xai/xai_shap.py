...
def build_ordinal_predict_fn(ordinal_erp_dataset, detector, ordinal_columns, **dataset_kwargs):
    # Create onehot dataset
    dataset_onehot = ERPDataset(train_path=ordinal_erp_dataset.train_path,
                                test_path=ordinal_erp_dataset.test_path,
                                numeric_preprocessing=ordinal_erp_dataset.numeric_preprocessing,
    ...
    def predict_fn(x):
        ...
        x = pd.DataFrame(data=x, columns=ordinal_columns)
        categorical_data = ordinal_erp_dataset.preprocessed_data["cat_prep"].inverse_transform(x.iloc[:, :42])
        ...
        return detector.score_samples(data.to_numpy())
...
def explain_anomalies(X_anomalous, predict_fn, X_benign, background, out_file_path=None, model_to_optimize=None):
    ...
    if background in ['kmeans', 'kmedoids']:
    ...
    elif background in ['zeros', 'mean', 'NN', 'booking_key_mean', 'booking_key_medoid', 'booking_key_NN', 'single_optimum']:
    ...
    elif background in ['full', 'optimized', 'optimized_batch_norm', 'reliable']:
        ...
        raise ValueError("Variable background not one of ['all', 'booking_key', 'booking_key_medoid', 'kNN', "
                         "'single_optimum', 'takeishi', 'full', 'reliable']")
