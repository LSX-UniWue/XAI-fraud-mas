...
def image_reference_points(background, X_expl, mvtec_data=None, predict_fn=None, device=None):
    ...
    elif background == 'reliable':
        # optimized with Haldar test for adversarial sample
        if predict_fn is None or device is None:
            raise ValueError("background 'reliable' requires predict_fn and device as input")
        from xai.automated_background_torch import find_reliable_reference
        reference_points = np.zeros(X_expl.shape)
        for i in range(X_expl.shape[0]):
            reference_points[i] = find_reliable_reference(data_point=X_expl[i].reshape((1, *X_expl.shape[1:])),
                                                          mask=np.zeros((1, *X_expl.shape[1:])),
                                                          predict_fn=functools.partial(predict_fn, output_cpu=False),
                                                          device=device)
