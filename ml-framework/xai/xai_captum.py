...
import pandas as pd
import torch
from captum.attr import DeepLift, DeepLiftShap, IntegratedGradients, LRP, Saliency, InputXGradient
...
def add_lrp_layer_rules(module):
    ...
    #     module.rule = lrp_rules.Alpha1_Beta0_Rule()  # TODO <-- the only one that makes a difference!
...
def add_rules_to_layers(model) -> None:
...
def explain_anomalies(X_anomalous, model, predict_fn, xai_type, out_path, target=0, reference_points=None, device='cuda'):
    ...
    elif xai_type == 'captum_deepshap':
        explainer = DeepLiftShap(predict_fn)
        additional_kwargs['baselines'] = None  # TODO: needs a tuple of multiple samples
    else:
    ...
    for sample in tqdm.tqdm(range(X_anomalous.shape[0])):
        ...
        elif reference_points is not None and xai_type in ['captum_intgrad', 'captum_deepshap']:  # reset reference
        ...
        if xai_type in ['captum_lrp']:  # lrp gives out absolute influence, need to turn negative since our anomaly score is in ]-inf,0]
            expl[sample] *= -1
            # measure convergence delta of LRP for goodness of fit
            delta = explainer.compute_convergence_delta(attributions=attr_dl, output=model(input_tensor))
            convergence_deltas.append(delta.detach().numpy()[0])

    if out_path is not None:
        if anom_cols is not None and anom_index is not None:
            pd.DataFrame(expl, columns=anom_cols, index=anom_index).to_csv(out_path)
        else:
            np.save(file=out_path, arr=expl)
