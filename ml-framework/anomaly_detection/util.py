...
def get_score_dict(scores, y_true):
    return {**get_mean_rank(scores=scores, y=y_true),
            **get_max_rank(scores=scores, y=y_true),
            **get_auc_pr_score(scores=scores, y=y_true),
...
def get_auc_pr_score(scores, y, bin_labels=False):
...
def get_auc_roc_score(scores, y, bin_labels=False):
    """Calculates auc_pr_score for fraud cases from _scores.csv df"""
    if not bin_labels:
        bin_labels = lambda y: (y != 'NonFraud').astype(int)  # binarize label for auc scores
        y = bin_labels(y)
